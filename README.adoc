...
The first two lines specify that you want to use the [hotspot=ear file=0]`ear` and [hotspot=liberty file=0]`liberty` plug-ins for Gradle. The [hotspot=earConfig file=0]`ear` section configures the `ear` task with the deployment descriptor that provides the web module file name and the context root as [hotspot=webModule file=0]`/converter`.
...
The [hotspot=libertyConfig file=0]`liberty` section configures the `liberty` task that creates the Liberty server name as [hotspot=sampleLibertyServer file=0]`sampleLibertyServer` and deploys the ear file from the path that is specified by the [hotspot=copyLibsDirectory file=0]`copyLibsDirectory` variable.
...
The `server.xml` configuration file configures with the [hotspot=EARdefinition file=1]`enterpriseApplication` element to specify the location of your EAR application.
...
The `settings.gradle` file is used to specify multiple modules that includes the [hotspot=includeJar hotspot=jarDir file=0]`jar`, [hotspot=includeWar hotspot=warDir file=0]`war`, and [hotspot=includeEar hotspot=earDir file=0]`ear` projects and their directories.
...
The [hotspot=getFeet file=0]`getFeet(cm)` invocation is added to the [hotspot=setHeightFeet file=0]`setHeightFeet` method to convert a measurement into feet.

The [hotspot=getInches file=0]`getInches(cm)` invocation is added to the [hotspot=setHeightInches file=0]`setHeightInches` method to convert a measurement into inches.
...
The [hotspot=getFeetMethod file=1]`getFeet` method is changed to convert the `cm` integer parameter from centimeters to feet, and the [hotspot=getInchesMethod file=1]`getInches` method to convert the `cm` integer parameter from centimeters to inches. The `sum`, `diff`, `product`, and `quotient` methods are updated to add, subtract, multiply, and divide 2 numbers respectively.
...
The [hotspot=test file=1]`test` task configuration is already included in the [hotspot file=1]`ear/build.gradle` file for you. It passes the [hotspot=httpPort file=1]`http.port` value to the tests so they can connect to the application during execution. In container dev mode the plug-in supplies the mapped host port via `liberty.http.port`; otherwise the value falls back to the port configured in `liberty.server.var`.
