...
If no context path is specified, Gradle automatically uses the WAR file artifact ID as the context root for the application, when the `application.xml` file is being generated. The default artifact ID is `{pojectName}-{moduleName}-{version}`, like `guide-gradle-multimodules-war-1.0-SNAPSHOT`.
...
You can now develop the application and the different modules together in dev mode by using the Liberty Gradle plug-in. To learn more about how to use dev mode with multiple modules, check out the https://github.com/OpenLiberty/ci.gradle/blob/main/docs/libertyDev.md#multiple-modules[Documentation^].
...
ifndef::cloud-hosted[]
Now check out the application again at the http://localhost:9080/converter/[http://localhost:9080/converter/^] URL. Try entering a height in centimeters and see whether converts correctly.
endif::[]
ifdef::cloud-hosted[]
